...
class Packagewise_booking_enquiry extends CI_Controller {
	...
	function __construct() {
        ...
        $this->module_url_path    =  base_url().$this->config->item('admin_panel_slug')."admin/packagewise_booking_enquiry";
        $this->module_title       = "Package Wise booking Enquiry Report";
        $this->module_url_slug    = "packagewise_booking_enquiry";
        $this->module_view_folder = "packagewise_booking_enquiry/";
        $this->arr_view_data = [];
	 }
        ...
        public function get_date(){ 
            ...
            {  
                ...
                if($package_title !='' && $from_date =='' && $to_date ==''){
                    ...
                    // print_r($arr_data); die;
                } else if($package_title !='' && $from_date !='' && $to_date !='' && empty($isChecked)){
                    // echo '2';
                    $record = array();
                    $fields = "booking_enquiry.*,packages.tour_title";
                    $this->db->where('booking_enquiry.agent_id',$package_title);
                    ...
                    if(in_array('All', $isChecked)){
                        ...
                        $this->db->join("packages", 'booking_enquiry.package_id=packages.id','left');
                        $arr_data = $this->master_model->getRecords('booking_enquiry');
                    } else if(in_array('Booking process', $isChecked)){
                        ...
                        $arr_data = $this->master_model->getRecords('booking_enquiry');
                        // print_r($arr_data); die;
                    } else if(in_array('Not Interested', $isChecked)){
