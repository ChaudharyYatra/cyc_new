...
class Ingredients_compare_module extends CI_Controller{
	...
	public function __construct()
	{
        ...
        $this->module_url_path    =  base_url().$this->config->item('admin_panel_slug')."/ingredients_compare_module";
        $this->module_title       = "Ingredients Compare Module";
        $this->module_url_slug    = "ingredients_compare_module";
        $this->module_view_folder = "ingredients_compare_module/";
	}
    ...
    {   
        ...
        $this->db->order_by('ingredients_data.id', 'desc');
        ...
        $this->db->having('COUNT(ingredients_data.ingredients_name) > 1');  // Filter to get only duplicates
        ...
        $fields = "ingredients_data.*, ingredients.ingredients, measuring_unit.unit_type,supervision.supervision_name";
        $this->db->order_by('ingredients_data.id', 'desc');
        $this->db->where('ingredients_data.is_deleted', 'no');
        $this->db->join("ingredients", 'ingredients_data.ingredients_name = ingredients.id', 'left');
        $this->db->join("supervision", 'ingredients_data.maharaj_id = supervision.id', 'left');
        $this->db->join("measuring_unit", 'ingredients_data.ingredients_weight = measuring_unit.id', 'left');

        // Subquery to find ingredients_name with more than one occurrence
        $this->db->where('ingredients_data.ingredients_name NOT IN (
            SELECT ingredients_name
            FROM ingredients_data
            WHERE is_deleted = "no"
            GROUP BY ingredients_name
            HAVING COUNT(ingredients_name) > 1
        )');

        $different_ingredient_name = $this->master_model->getRecords('ingredients_data', array('ingredients_data.is_deleted' => 'no'), $fields);
        // print_r($different_ingredient_name); die;

        // $this->arr_view_data['supervision_sess_name'] = $supervision_sess_name;
        $this->arr_view_data['action']          = 'add';
        $this->arr_view_data['same_ingredient_name'] = $same_ingredient_name;
        $this->arr_view_data['different_ingredient_name'] = $different_ingredient_name;
