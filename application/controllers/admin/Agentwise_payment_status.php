...
class Agentwise_payment_status extends CI_Controller {
	...
	function __construct() {
        ...
        $this->module_url_path    =  base_url().$this->config->item('admin_panel_slug')."admin/agentwise_payment_status";
        $this->module_title       = "Agentwise Payment Status Report";
        $this->module_url_slug    = "agentwise_payment_status";
        $this->module_view_folder = "agentwise_payment_status/";
        $this->arr_view_data = [];
	 }
        ...
        public function get_date(){ 
            ...
            public function agentwise_payment()  
            {  
                ...
                // print_r($isChecked); die;
                $arr_data =array();
                if($agent_name !='' && $from_date =='' && $to_date ==''){
                    // echo '1';
                    $record = array();
                    $fields = "booking_enquiry.first_name,booking_enquiry.last_name,booking_enquiry.email,booking_enquiry.mobile_number,
                    booking_enquiry.gender,packages.tour_title,booking_payment_details.payment_confirmed_status,booking_payment_details.final_amt,
                    booking_payment_details.booking_amt";
                    $this->db->where('booking_enquiry.agent_id',$agent_name);
                    $this->db->join("packages", 'booking_payment_details.package_id=packages.id','left');
                    $this->db->join("booking_enquiry", 'booking_payment_details.enquiry_id=booking_enquiry.id','left');
                    $this->db->join("agent", 'booking_enquiry.agent_id=agent.id','left');
                    $this->db->group_by('booking_payment_details.enquiry_id'); 
                    $arr_data = $this->master_model->getRecords('booking_payment_details',array('booking_payment_details.is_deleted'=>'no'),$fields);
                    // print_r($arr_data); die;

                } else if($agent_name !='' && $from_date !='' && $to_date !='' && empty($isChecked)){
                    // echo '2';
                    $record = array();
                    $fields = "booking_enquiry.first_name,booking_enquiry.last_name,booking_enquiry.email,booking_enquiry.mobile_number,
                    booking_enquiry.gender,packages.tour_title,booking_payment_details.payment_confirmed_status,booking_payment_details.final_amt,
                    booking_payment_details.booking_amt";
                    $this->db->where('booking_enquiry.agent_id',$agent_name);
                    $this->db->where("booking_payment_details.created_at BETWEEN '$from_date' AND '$to_date'", NULL, FALSE);
                    $this->db->join("packages", 'booking_payment_details.package_id=packages.id','left');
                    $this->db->join("booking_enquiry", 'booking_payment_details.enquiry_id=booking_enquiry.id','left');
                    $this->db->join("agent", 'booking_enquiry.agent_id=agent.id','left');
                    $this->db->group_by('booking_payment_details.enquiry_id'); 
                    ...
                    if(in_array('All', $isChecked)){
                        ...
                        $arr_data = $this->master_model->getRecords('booking_payment_details',array('booking_payment_details.is_deleted'=>'no'),$fields);
                        // print_r($arr_data); die;
                    } else if(in_array('Final Payment', $isChecked)){
                        // echo 'Booking processsssssssss';
                        $record = array();
                        $fields = "booking_enquiry.first_name,booking_enquiry.last_name,booking_enquiry.email,booking_enquiry.mobile_number,
                        booking_enquiry.gender,packages.tour_title,booking_payment_details.payment_confirmed_status,booking_payment_details.final_amt,
                        booking_payment_details.booking_amt";
                        $this->db->where('booking_enquiry.agent_id',$agent_name);
                        $this->db->where('booking_payment_details.payment_confirmed_status','Payment Completed');
                        $this->db->where("booking_payment_details.created_at BETWEEN '$from_date' AND '$to_date'", NULL, FALSE);
                        $this->db->join("packages", 'booking_payment_details.package_id=packages.id','left');
                        $this->db->join("booking_enquiry", 'booking_payment_details.enquiry_id=booking_enquiry.id','left');
                        $this->db->join("agent", 'booking_enquiry.agent_id=agent.id','left');
                        ...
                        $arr_data = $this->master_model->getRecords('booking_payment_details',array('booking_payment_details.is_deleted'=>'no'),$fields);
                        // print_r($arr_data); die;
                    } else if(in_array('In Process Of Payment', $isChecked)){
                        // echo 'Not Interesteddddddddddddddd';
                        $record = array();
                        $fields = "booking_enquiry.first_name,booking_enquiry.last_name,booking_enquiry.email,booking_enquiry.mobile_number,
                        booking_enquiry.gender,packages.tour_title,booking_payment_details.payment_confirmed_status,booking_payment_details.final_amt,
                        booking_payment_details.booking_amt";
                        $this->db->where('booking_enquiry.agent_id',$agent_name);
                        $this->db->where('booking_payment_details.payment_confirmed_status','In Process');
                        $this->db->where("booking_payment_details.created_at BETWEEN '$from_date' AND '$to_date'", NULL, FALSE);
                        $this->db->join("packages", 'booking_payment_details.package_id=packages.id','left');
                        $this->db->join("booking_enquiry", 'booking_payment_details.enquiry_id=booking_enquiry.id','left');
                        $this->db->join("agent", 'booking_enquiry.agent_id=agent.id','left');
                        ...
                        // echo 'booking doneeeeeeeeeeeeeeeee';
                        $record = array();
                        $fields = "booking_enquiry.first_name,booking_enquiry.last_name,booking_enquiry.email,booking_enquiry.mobile_number,
                        booking_enquiry.gender,packages.tour_title,booking_payment_details.payment_confirmed_status,booking_payment_details.final_amt,
                        booking_payment_details.booking_amt";
                        $this->db->where('booking_enquiry.agent_id',$agent_name);
                        $this->db->where('booking_payment_details.payment_confirmed_status','Payment Not Paid');
                        $this->db->where("booking_payment_details.created_at BETWEEN '$from_date' AND '$to_date'", NULL, FALSE);
                        $this->db->join("packages", 'booking_payment_details.package_id=packages.id','left');
                        $this->db->join("booking_enquiry", 'booking_payment_details.enquiry_id=booking_enquiry.id','left');
                        $this->db->join("agent", 'booking_enquiry.agent_id=agent.id','left');
            ...
            //         $record = array();
            //         $fields = "booking_enquiry.*,packages.tour_title";
            //         $this->db->where('booking_enquiry.agent_id',$agent_name);
            //         $this->db->where("booking_enquiry.booking_date BETWEEN '$from_date' AND '$to_date'", NULL, FALSE);
            //         $this->db->join("agent", 'booking_enquiry.agent_id=agent.id','left');
