...
class Agentwise_check_status_enquiry extends CI_Controller {
	...
	function __construct() {
        ...
        $this->module_url_path    =  base_url().$this->config->item('admin_panel_slug')."admin/agentwise_check_status_enquiry";
        $this->module_title       = "Agent Wise check status Enquiry Report";
        $this->module_url_slug    = "agentwise_check_status_enquiry";
        $this->module_view_folder = "agentwise_check_status_enquiry/";
        $this->arr_view_data = [];
	 }
        ...
        //    print_r($data); die;
            ...
            public function check_status_enquiry()  
            {  
                ...
                if($agent_name !='' && $from_date =='' && $to_date ==''){
                    ...
                    // print_r($arr_data); die;
                } else if($agent_name !='' && $from_date !='' && $to_date !='' && $enquiry_status ==''){
                    // echo '2';
                    $record = array();
                    $fields = "booking_enquiry.*,packages.tour_title";
                    $this->db->where('booking_enquiry.agent_id',$agent_name);
                    ...
                    if($enquiry_status =='New Enquiry'){
                        ...
                        $this->db->join("packages", 'booking_enquiry.package_id=packages.id','left');
                        $arr_data = $this->master_model->getRecords('booking_enquiry');
                    } else if($enquiry_status =='Pending Enquiry'){
                        ...
                        $record = array();
                        $fields = "booking_enquiry.*,packages.tour_title,booking_payment_details.enquiry_id";
                        $this->db->where('booking_enquiry.agent_id',$agent_name);
                        $this->db->where("booking_enquiry.booking_date BETWEEN '$from_date' AND '$to_date'", NULL, FALSE);
                        ...
                        $this->db->join("booking_payment_details", 'booking_enquiry.id=booking_payment_details.enquiry_id','left');
                        // $this->db->join("final_booking", 'booking_enquiry.id=final_booking.enquiry_id','left');
                        $this->db->join("agent", 'booking_enquiry.agent_id=agent.id','left');
                        $this->db->join("packages", 'booking_enquiry.package_id=packages.id','left');
