...
class Tm_request_more_fund extends CI_Controller{
	...
	public function __construct()
	{
        ...
        $this->module_title       = "Request to Tour Operation Manager";
    ...
    {   
        ...
        if($this->input->post('submit'))
        {
                ...
                $priority_status = trim($this->input->post('priority_status'));
                ...
                if($inserted_id > 0)
                {    
                    $this->session->set_flashdata('success_message',"Request Record Added Successfully.");
...
public function active_inactive($id,$type)
{
  $id=base64_decode($id);
    ...
    public function Received_money_approval($id)
    {
        $supervision_sess_name = $this->session->userdata('supervision_name');
        $iid = $this->session->userdata('supervision_sess_id');
        ...
        $fields = "tm_request_more_fund.*, 
                ...
                supervision_tom.supervision_name AS tom_supervision_name,agent.agent_name";
        $this->db->where('tm_request_more_fund.is_deleted', 'no');
        $this->db->where('tm_request_more_fund.id', $id);
        ...
        $this->db->join("agent", 'tm_request_more_fund.amt_send_agent_id = agent.id', 'left');
        $arr_data = $this->master_model->getRecords('tm_request_more_fund', array('tm_request_more_fund.is_deleted' => 'no'), $fields);
        // print_r($arr_data); die;
        
        $this->arr_view_data['arr_data']        = $arr_data;
        $this->arr_view_data['supervision_sess_name'] = $supervision_sess_name;
        $this->arr_view_data['page_title']      = "Received_money_approval ".$this->module_title;
        $this->arr_view_data['module_title']    = $this->module_title;
        $this->arr_view_data['module_url_path'] = $this->module_url_path;
        $this->arr_view_data['middle_content']  = $this->module_view_folder."Received_money_approval";
        $this->load->view('tour_manager/layout/agent_combo',$this->arr_view_data);
    }
