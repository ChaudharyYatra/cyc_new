...
class Ctv_transfer_receipt extends CI_Controller {
	...
	function __construct() {
        ...
        $this->module_url_path    =  base_url().$this->config->item('agent_panel_slug')."/ctv_transfer_receipt";
        ...
        $this->payment_other_than_sra    =  base_url().$this->config->item('agent_panel_slug')."/payment_other_than_sra";
        $this->module_title       = "CTV Transfer Receipt";
        $this->module_ursl_slug    = "ctv_transfer_receipt";
        $this->module_view_folder = "ctv_transfer_receipt/";
        $this->arr_view_data = [];
	 }
     ...
     public function index($iid)
     {
        ...
        $record = array();
        $fields = "payment_other_than_sra.*, 
           agent1.agent_name as ag_name,agent1.booking_center as booking_center, 
           agent2.agent_name as staff_name";

        $this->db->where('payment_other_than_sra.is_deleted', 'no');
        $this->db->where('payment_other_than_sra.id', $iid);

        // Join for agent_id
        $this->db->join("agent as agent1", 'agent1.id = payment_other_than_sra.agent_id', 'left');

        // Join for staff_name
        $this->db->join("agent as agent2", 'agent2.id = payment_other_than_sra.staff_name', 'left');

        $arr_data = $this->master_model->getRecord('payment_other_than_sra', array('payment_other_than_sra.is_deleted' => 'no'), $fields);
        // print_r($arr_data); die;


        $payment_rupee = $arr_data['amount'];
        ...
        // print_r($pay); die;
         ...
         $this->arr_view_data['payment_other_than_sra'] = $this->payment_other_than_sra;
