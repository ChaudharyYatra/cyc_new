...
class Sra_payment_record extends CI_Controller{
	...
	public function index()
	{
        ...
        $fields = "sra_payment.*,sra_booking_payment_details.booking_amt,academic_years.year,packages.tour_number as package_tour_number,package_date.journey_date";
        // $this->db->order_by('id','ASC');
        $this->db->where('sra_booking_payment_details.is_deleted','no');
        $this->db->where('sra_payment.is_deleted','no');
        $this->db->where('sra_booking_payment_details.agent_id',$id);
        $this->db->join("packages", 'packages.id=sra_booking_payment_details.package_id','left');
