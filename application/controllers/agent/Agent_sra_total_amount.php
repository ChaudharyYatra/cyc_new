...
class Agent_sra_total_amount extends CI_Controller {
	...
	function __construct() {
        ...
        $this->module_url_path    =  base_url().$this->config->item('agent_panel_slug')."/agent_sra_total_amount";
        $this->module_url_path_submit_next   =  base_url().$this->config->item('agent_panel_slug')."/srs_form";
        $this->module_title       = "Payment Received Data";
        $this->module_view_folder = "agent_sra_total_amount/";
        $this->arr_view_data = [];
	 }
     ...
     public function index()
     {
        ...
        $this->db->select('select_transaction, SUM(booking_amt) as total_amount'); // Replace amount_field with the actual field you want to sum
        $this->db->group_by('select_transaction'); 
        $arr_data = $this->master_model->getRecords('sra_booking_payment_details');
        //print_r($arr_data); die;

        // $this->db->where('is_deleted','no');
        // $this->db->group_by('sra_booking_payment_details.select_transaction'); 
        // $arr_data = $this->master_model->getRecords('sra_booking_payment_details');
        // //print_r($arr_data); die;
     ...
     public function details($iid)
     {
        ...
        package_date.id as p_date_id,booking_payment_details.package_id,booking_payment_details.package_date_id,
    ...
    public function sub_index($pid,$p_date_id)
     {
        $agent_sess_name = $this->session->userdata('agent_name');
        ...
        // print_r($iid); die;
         ...
         $this->db->where('final_booking.package_date_id',$p_date_id);
         $this->db->where('booking_payment_details.agent_id',$iid);
         $this->db->where('all_traveller_info.for_credentials','yes');
