...
table.scrolldown tbody{
    ...
    <section class="trending pt-6 pb-0 bg-lgrey" id="data-table">
        <div class="container">
            ...
            <div class="pagination-container">
                <nav>
                    <ul class="pagination justify-content-center">
                        ...
                        // Previous Button
                        $prev_start = max($start - $limit, 0); // Ensure start is not negative
                        $prev_disabled = ($start == 0) ? 'disabled' : '';
                        echo "<li class='page-item $prev_disabled'><a class='page-link' href='" . base_url() . "packages/all_packages?start=$prev_start&limit=$limit'>Previous</a></li>";
            
                        // Page Numbers
                        for ($i = 1; $i <= $total_pages; $i++) {
                            $active = (($start / $limit) + 1 == $i) ? 'active' : '';
                            $page_start = ($i - 1) * $limit;
                            echo "<li class='page-item $active'><a class='page-link' href='" . base_url() . "packages/all_packages?start=$page_start&limit=$limit'>$i</a></li>";
                        }
            
                        // Next Button
                        $next_start = min($start + $limit, ($total_pages - 1) * $limit); // Ensure start does not exceed total
                        $next_disabled = (($start / $limit) + 1 >= $total_pages) ? 'disabled' : '';
                        echo "<li class='page-item $next_disabled'><a class='page-link' href='" . base_url() . "packages/all_packages?start=$next_start&limit=$limit'>Next</a></li>";
                        ?>
                    </ul>
                </nav>
            </div>
